...
test('test', async ({ page }) => {
  ...
  await page.locator('div').filter({ hasText: /^Next$/ }).first().click();
  await page.getByLabel('I consent to sharing my information with the relevant reporting agencies in case of multiple discrimination, in an anonymized form (hyperlink to the list).').check();
  await page.getByLabel('I hereby confirm the accuracy of my information, acknowledge the data protection regulations (hyperlink to data protection concept), and consent to the described processing of my personal data.*').check();
